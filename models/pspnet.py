...
2. I omitted auxilary loss to keep consistency of data loader
"""
import torch
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, dilation=1, downsample=None):
        ...
        self.conv2 = conv3x3(inplanes, planes, dilation=dilation)
...
def drn_resnet50(pretrained=True):
    ...
    if pretrained:
        load_weights(model, model_zoo.load_url(model_urls['resnet50']))
    return model

class PSPModule(nn.Module):
    def __init__(self, in_dim, out_dim, sizes=(1, 2, 3, 6), pool_type='avg'):
        """
    ...
    def _make_stage(self, size):
        ...
        if self.pool_type == 'avg':
            ...
            layers.append(nn.AdaptiveMaxPool2d((size, size)))
        layers.append(nn.Conv2d(self.in_dim, self.pyramid_dim, kernel_size=1, bias=False))
        return nn.Sequential(*layers)
...
class PSPUpsample(nn.Module):
    """ Upsampling dim => dim/2 & size => 2*size -- Restore """
...
class PSPNet(nn.Module):
    ...
    def forward(self, input):
        ...
        assert out.size(2) == h and out.size(3) == w, 'Size mismatched {} == {}? / {} == {}?' \
                                                       .format(out.size(2), h, out.size(3), w)
        return out

if __name__ == '__main__':
    model = PSPNet(21)
    input = torch.randn(2 ,3, 512, 256)
